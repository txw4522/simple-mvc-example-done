...
const setNameDog = (req, res) => {
  ...
  // normally you would also perform validation
  // to know if the data they sent you was real
  if (!req.body.firstname || !req.body.lastname || !req.body.breeds || !req.body.age) {
    // if not respond with a 400 error
    // (either through json or a web page depending on the client dev)
  ...
  savePromise.then(() => {
    ...
    lastAddedDog = newDog;
    // return success
    res.json({ name: lastAddedDog.name, breed: lastAddedDog.dogBreed, age: lastAddedCat.age });
  });
